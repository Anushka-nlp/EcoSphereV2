...
class _FullscreenImageViewerState extends State<FullscreenImageViewer> {
    ...
    try {
      ...
      if (Platform.isAndroid) {
        ...
        try {
          ...
          if (!mounted) return;
          snackBar("Saved to Downloads/EchoSphere/$fileName");
        } catch (e) {
          // Fallback
          ...
          await Share.shareXFiles([XFile(file.path)],
              text: "Image shared from EchoSphere");
        }
        ...
        final saveDir = Directory('$path/EchoSphere');
        ...
        await file.writeAsBytes(bytes);
        if (!mounted) return;
        snackBar("Saved to $path/EchoSphere");
      }
    } catch (e) {
      if (mounted) {
  ...
  Future<void> _shareImage(String url) async {
    ...
    try {
      ...
      final fileName =
          "echosphere_share_${DateTime.now().millisecondsSinceEpoch}.$extension";
      final tempDir = await getTemporaryDirectory();
      final file = File('${tempDir.path}/$fileName');
      await file.writeAsBytes(bytes);

      await Share.shareXFiles([XFile(file.path)], text: "Shared from EchoSphere");
    } catch (e) {
      if (mounted) {
                ...
                child: ClipRect(
                  child: InteractiveViewer(
                    ...
                    child: Hero(
                      ...
                      flightShuttleBuilder: EchoSphereImage.heroFlightShuttleBuilder,
                      child: EchoSphereImage(
