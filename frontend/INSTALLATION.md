# Installation Guide for EchoSphere
...
| **Linux ([AUR](https://aur.archlinux.org/))** | You're on Arch Linux — use [`echosphere-bin`](https://aur.archlinux.org/packages/echosphere-bin). |
...
1. **Install Chocolatey (if not already):**
   ...
   ```
2. **Install EchoSphere:**
   In Command Prompt or PowerShell, run:
   ```powershell
   choco install com.ryan.echosphere
   ```
3. **Update later with:**
   In Command Prompt or PowerShell, run:
   ```powershell
   choco upgrade com.ryan.echosphere
   ```
---
...
3. **Install the app:**
   In Command Prompt or PowerShell, run:
   ```powershell
   scoop install echosphere
   ```
4. **Update later with:**
   In Command Prompt or PowerShell, run:
   ```powershell
   scoop update echosphere
   ```
---

### 🔹 Windows (ZIP or Installer)
- **ZIP**: Download `EchoSphere-Windows.zip`, extract, and run `echosphere.exe`.
- **Installer**: Use `EchoSphere-x86_64-<version>-Installer.exe` for full setup.
---

### 🔹 Linux
- **AppImage**: Make it executable and run:
  ```bash
  chmod +x EchoSphere.AppImage
  ./EchoSphere.AppImage
  ```
